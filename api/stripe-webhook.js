...
async function sendEmails(orderData) {
  ...
  const transporter = nodemailer.createTransport({
    host: "smtp.strato.de", // SMTP-Server (Passe diesen ggf. an)
...
app.post("/api/stripe-webhook", async (req, res) => {
  ...
  // Wenn die Checkout-Session abgeschlossen wurde
  if (event.type === "checkout.session.completed") {
    const session = event.data.object;
    console.log("✅ Zahlung abgeschlossen:", session);

    // Bereite die Bestelldaten vor. Hier werden Metadaten (z.B. der Warenkorb) genutzt.
    const orderData = {
